...
class TopStoriesViewController: ViewController {
    ...
    func bindData() {
        guard let viewModel = viewModel as? TopStoriesViewModel else {
            print("CANT LOAD VM")
            return
        }
        ...
        let input = TopStoriesViewModel.Input(fetchTopStoriesEvent:  Driver.just(()), selectedArticleEvent: selectedArticle)
...
private extension Reactive where Base: TopStoriesViewController {
