...
extension Driver {
    func applySkipFirst(_ shouldSkipFirst: Bool) -> SharedSequence<SharingStrategy, Element> {
        shouldSkipFirst ? skip(1) : asSharedSequence()
    }
}
...
public extension Reactive where Base: UIScrollView {
    var reachedBottom: Observable<Void> {
        ...
        return contentOffset.flatMap { [weak scrollView] contentOffset -> Observable<Void> in
            guard let scrollView else { return Observable.empty() }
...
func <-> (textInput: TextInput<some Any>, variable: BehaviorRelay<String>) -> Disposable {
    ...
    let bindToVariable = textInput.text
        .subscribe(onNext: { [weak base = textInput.base] _ in
             ...
             */
            if let nonMarkedTextValue, nonMarkedTextValue != variable.value {
